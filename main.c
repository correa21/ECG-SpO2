...
void beat (void)
{
	printf("Spo2: %d\n",SpO2Calculator_getSpO2());
	/*SEND SPO2 DATA BY SERIAL*/
};
typedef enum {
	WAITING_CARD,
	DOING_EXAM
...
int main(void)
{
	...
	//ECG_ADC_Init();
	//RFID_module_init();
	sistem_state_t state = DOING_EXAM;
	BooleanType valid_user = FALSE;
	BooleanType exam_done = FALSE;
	for(;;)
	{
		switch(state)
		{
			case WAITING_CARD:
				...
				while(FALSE == valid_user)
				{
					/*INCERT READING CARD PROTOCOL*/
				}

				/*SEND BY SERIAL THE DATA ASOCIATED WITH THE CARD*/
				state = DOING_EXAM;
			break;

			case DOING_EXAM:
				while(FALSE == exam_done)
				{
					MAX30100_pulseOximeter_update();//METER EN UN PIT?
					/*ADC SQUENCE*/
					/*SEND SERIAL*/
					//state = WAITING_CARD;
				}
